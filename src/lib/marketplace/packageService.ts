...
export async function getPackages(vendorId: string): Promise<VendorPackageDocument[]> {
  if (!db) return [];
  try {
    const snap = await getDocs(query(collection(db, collections.vendorPackages), where("vendorId", "==", vendorId), orderBy("createdAt", "desc")));
    return snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<VendorPackageDocument, "id">) }));
  } catch { return []; }
}
...
export async function getGallery(galleryId: string): Promise<VendorGalleryDocument[]> {
  if (!db) return [];
  try {
    const snap = await getDocs(query(collection(db, collections.vendorGallery), where("vendorId", "==", galleryId), orderBy("createdAt", "desc")));
    return snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<VendorGalleryDocument, "id">) }));
  } catch { return []; }
}
