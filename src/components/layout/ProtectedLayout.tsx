...
import {
  Heart, LayoutDashboard, User, Search, Sparkles, Star,
  HeartHandshake, Bell, Crown, Settings, LogOut, Menu, X,
} from "lucide-react";
    ...
    <div className="flex min-h-screen bg-grain">
      ...
      <div className="flex flex-1 flex-col lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-primary-100 bg-background/90 px-4 backdrop-blur-md lg:px-8">
          <button type="button" onClick={() => setOpen(true)} className="rounded-full p-2 text-primary-900 lg:hidden" aria-label="Open menu">
            <Menu className="h-5 w-5" />
          </button>
          <Link href="/" className="flex items-center gap-2.5" aria-label="WedBridge home">
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-white shadow-soft">
...
function SidebarContent({
  pathname, t, onLogout, userName, onClose,
}: {
  pathname: string;
  t: (k: never) => string;
  onLogout: () => void;
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between gap-2 px-5 py-5">
        ...
        {onClose && (
          <button type="button" onClick={onClose} className="rounded-full p-1.5 text-primary-900" aria-label="Close menu">
            <X className="h-5 w-5" />
          </button>
        )}
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? "bg-primary-50 text-primary-900" : "text-ink/70 hover:bg-primary-50/60 hover:text-primary-900",
              )}
            >
      ...
      <div className="border-t border-primary-100 p-3">
        <button type="button" onClick={onLogout} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-ink/70 transition hover:bg-accent-50 hover:text-accent-700">
          <LogOut className="h-4 w-4" />
