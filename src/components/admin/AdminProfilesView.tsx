...
export function AdminProfilesView() {
  ...
  const toggleVerified = async (p: ProfileDocument) => { setActing(p.id); await updateProfile(p.id, { verified: !p.verified }); setActing(null); toast("Updated", "success"); load(); };
  const toggleFeatured = async (p: ProfileDocument) => { setActing(p.id); await updateProfile(p.id, { featured: !p.featured }); setActing(null); toast("Updated", "success"); load(); };
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p) => (
            <div key={p.id} className="card p-4">
              <div className="flex items-center gap-3"><div className="h-12 w-12 overflow-hidden rounded-xl bg-primary-100">{p.photoURL && <img src={p.photoURL} alt={p.name} className="h-full w-full object-cover" />}</div><div className="flex-1"><h3 className="font-semibold text-primary-900">{p.name}</h3><p className="text-xs text-muted">{p.religion} · {p.caste ?? ""}</p><p className="text-xs text-muted">{formatDate(p.createdAt as unknown as string)}</p></div></div>
