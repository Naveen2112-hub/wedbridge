...
const navItems = [
  { href: "/admin/dashboard", key: "admin.dashboard.title" as const, icon: LayoutDashboard }, { href: "/admin/users", key: "admin.users" as const, icon: Users },
  { href: "/admin/profiles", key: "admin.profiles" as const, icon: FileText }, { href: "/admin/vendors", key: "admin.vendors" as const, icon: Store },
  { href: "/admin/memberships", key: "admin.memberships" as const, icon: Crown }, { href: "/admin/settings", key: "admin.settings" as const, icon: Settings },
];
...
function SidebarContent({ pathname, t, onLogout, onClose }: { pathname: string; t: (k: never) => string; onLogout: () => void; onClose?: () => void; }) {
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between gap-2 px-5 py-5"><Link href="/admin/dashboard" className="flex items-center gap-2.5" onClick={onClose}><span className="flex h-9 w-9 items-center justify-center rounded-full bg-white/15 text-white"><Shield className="h-4 w-4" /></span><span className="font-display text-lg font-semibold text-white">Admin</span></Link>{onClose && <button type="button" onClick={onClose} className="rounded-full p-1.5 text-white" aria-label="Close menu"><X className="h-5 w-5" /></button>}</div>
      <nav className="flex-1 space-y-1 px-3 py-2">{navItems.map((item) => { const active = pathname === item.href || pathname.startsWith(item.href + "/"); return (<Link key={item.href} href={item.href} onClick={onClose} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition", active ? "bg-white/15 text-white" : "text-white/70 hover:bg-white/10 hover:text-white")}><item.icon className="h-4 w-4" />{t(item.key as never)}</Link>); })}</nav>
      <div className="border-t border-white/10 p-3"><button type="button" onClick={onLogout} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-white/70 transition hover:bg-white/10 hover:text-white"><LogOut className="h-4 w-4" />{t("nav.logout" as never)}</button></div>
    </div>
  );
}
void Heart;
