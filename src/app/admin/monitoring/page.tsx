...
export default function MonitoringPage() {
  ...
  const categoryColors: Record<string, string> = {
    ...
    permission: "text-red-600 bg-red-50",
    quota: "text-orange-600 bg-orange-50",
    "not-found": "text-blue-600 bg-blue-50",
    validation: "text-purple-600 bg-purple-50",
        ...
        <>
          <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="card p-5">
              <div className="flex items-center gap-2 text-sm text-gray-500"><ShieldCheck className="h-4 w-4" />Total Errors</div>
              <p className="mt-2 font-display text-2xl font-bold text-primary-900">{summary?.total ?? 0}</p>
            </div>
            {Object.entries(summary?.byCategory ?? {}).map(([cat, count]) => (
              <div key={cat} className="card p-5">
                <div className={`flex items-center gap-2 text-sm ${categoryColors[cat] ?? "text-gray-600"}`}><AlertTriangle className="h-4 w-4" />{cat}</div>
                <p className="mt-2 font-display text-2xl font-bold text-primary-900">{count}</p>
              </div>
            ))}
          </div>
            ...
            <div className="card mb-6 p-6">
              ...
              <div className="space-y-2">
                {summary.topErrors.map((err) => (
                  <div key={err.key} className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-2">
                    <span className="text-sm font-mono text-gray-700">{err.key}</span>
                    <span className="rounded-lg bg-red-50 px-2 py-1 text-xs font-medium text-red-600">{err.count}x</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="card p-6">
            ...
            {errors.length === 0 ? (
              <p className="text-sm text-gray-500">No errors recorded. Firebase is operating normally.</p>
            ) : (
