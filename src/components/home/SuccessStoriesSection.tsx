...
export function SuccessStoriesSection() {
  const { t } = useLanguage();
  return (<Section className="bg-primary-50/30"><div className="mx-auto max-w-2xl text-center"><h2 className="heading-lg">{t("home.success.title")}</h2><p className="text-lead mt-3">{t("home.success.subtitle")}</p></div><div className="mt-12 grid gap-6 md:grid-cols-3">{stories.map((s, i) => (<Reveal key={s.names} delay={i * 0.1}><div className="overflow-hidden rounded-2xl bg-card shadow-soft"><div className="relative aspect-[4/3] overflow-hidden"><Image src={s.image} alt={s.names} fill sizes="(min-width: 768px) 33vw, 100vw" className="object-cover" /></div><div className="p-6"><div className="flex gap-1 text-secondary-500">{Array.from({ length: 5 }).map((_, idx) => <Star key={idx} className="h-4 w-4" fill="currentColor" />)}</div><p className="mt-3 text-sm text-ink/80">&ldquo;{s.quote}&rdquo;</p><p className="mt-3 font-display text-sm font-semibold text-primary-900">{s.names}</p></div></div></Reveal>))}</div></Section>);
}
