...
export function ServicesSection() {
  ...
  return (
    <Section className="bg-gradient-to-b from-background to-primary-50/30">
      <SectionHeader eyebrow={t("home.services.eyebrow")} title={t("home.services.title")} subtitle={t("home.services.subtitle")} />
          ...
          <Reveal key={s.titleKey} delay={i * 0.08}>
            <article className="group h-full overflow-hidden rounded-2xl bg-card shadow-card transition hover:-translate-y-1 hover:shadow-glow">
              <div className="relative aspect-[4/3] overflow-hidden">
                <Image src={s.image} alt={t(s.titleKey)} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover transition duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-primary-950/60 via-transparent to-transparent" />
                <span className="absolute left-3 top-3 flex h-10 w-10 items-center justify-center rounded-xl bg-white/90 text-primary-800 shadow-soft"><s.icon className="h-5 w-5" /></span>
                <h3 className="absolute bottom-3 left-3 right-3 font-display text-lg font-semibold text-white">{t(s.titleKey)}</h3>
              </div>
