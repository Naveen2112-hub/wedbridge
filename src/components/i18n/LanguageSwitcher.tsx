  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-1.5 rounded-full border border-primary-100 bg-white/70 px-3 py-2 text-xs font-semibold text-primary-900 transition hover:border-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary"
          ...
          {languages.map((l) => (
            <li key={l.code}>
              ...
              >
                ...
                {language === l.code && <Check className="h-4 w-4 text-secondary-600" />}
              </button>
            </li>
          ))}
        </ul>
