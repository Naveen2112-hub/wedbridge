...
const statusStyles: Record<string, string> = { pending: "bg-amber-50 text-amber-700", paid: "bg-green-50 text-green-700", failed: "bg-red-50 text-red-700", cancelled: "bg-muted/20 text-muted", refunded: "bg-purple-50 text-purple-700", expired: "bg-muted/20 text-muted" };
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="overflow-x-auto rounded-2xl bg-card shadow-card">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-primary-50 text-left text-muted"><th className="p-3">User</th><th className="p-3">Plan</th><th className="p-3">Status</th><th className="p-3">Expiry</th></tr></thead>
                <tbody>
                  {subs.length === 0 ? <tr><td colSpan={4} className="p-6 text-center text-muted">No subscriptions yet.</td></tr> :
                  subs.map((s) => (
                    <tr key={s.id} className="border-b border-primary-50 last:border-0">
                      ...
                      <td className="p-3"><span className={cn("badge", s.status === "active" ? "bg-green-50 text-green-700" : "bg-muted/20 text-muted")}>{s.status}</span></td>
                      <td className="p-3 text-xs text-muted">{s.endDate ? new Date(s.endDate as unknown as number).toLocaleDateString() : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div>
            ...
            <div className="overflow-x-auto rounded-2xl bg-card shadow-card">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-primary-50 text-left text-muted"><th className="p-3">Order ID</th><th className="p-3">Plan</th><th className="p-3">Amount</th><th className="p-3">Status</th><th className="p-3">Action</th></tr></thead>
                <tbody>
          ...
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              {Object.keys(stats?.byPlan ?? {}).length === 0 && <p className="text-sm text-muted">No revenue data yet.</p>}
            </div>
          </div>
        </div>
      )}
...
function StatCard({ icon: Icon, label, value, color }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string; color: string }) {
  return (
    <div className="rounded-2xl bg-card p-5 shadow-card">
      <div className="flex items-center gap-3"><span className={cn("flex h-10 w-10 items-center justify-center rounded-xl bg-primary-50", color)}><Icon className="h-5 w-5" /></span><p className="text-sm text-muted">{label}</p></div>
