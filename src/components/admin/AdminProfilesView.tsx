...
export function AdminProfilesView() {
  ...
  return (
    <div>
      ...
      <div className="mb-4 relative max-w-sm"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-500" /><input className="input pl-10" placeholder="Search profiles…" value={search} onChange={(e) => setSearch(e.target.value)} /></div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p) => (
            <div key={p.id} className="card p-4">
              <div className="flex items-center gap-3"><div className="relative h-12 w-12 overflow-hidden rounded-xl bg-primary-100">{p.photoURL && <Image src={p.photoURL} alt={`${p.name} profile photo`} fill sizes="48px" className="object-cover" />}</div><div className="flex-1"><h3 className="font-semibold text-primary-900">{p.name}</h3><p className="text-xs text-gray-500">{p.religion} · {p.caste ?? ""}</p><p className="text-xs text-gray-500">{formatDate(p.createdAt as unknown as string)}</p></div></div>
