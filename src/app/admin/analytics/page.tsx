...
import { Users, Eye, Sparkles, Send, Crown, Store } from "lucide-react";
...
export default function AdminAnalyticsPage() {
  ...
  const cards = [
    ...
    { label: "Vendor Revenue", value: formatCurrency(data.vendorRevenue), icon: Store, color: "text-green-600 bg-green-50" },
  ];
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {cards.map((c) => <div key={c.label} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-primary-100 transition hover:shadow-md"><span className={cn("flex h-10 w-10 items-center justify-center rounded-xl", c.color)}><c.icon className="h-5 w-5" /></span><p className="mt-3 font-display text-2xl font-bold text-primary-900">{c.value}</p><p className="text-sm text-muted">{c.label}</p></div>)}
      </div>
    </div>
  );
}
