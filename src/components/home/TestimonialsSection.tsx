...
export function TestimonialsSection() {
  const { t } = useLanguage();
  return (<Section><div className="mx-auto max-w-2xl text-center"><h2 className="heading-lg">{t("home.testimonials.title")}</h2><p className="text-lead mt-3">{t("home.testimonials.subtitle")}</p></div><div className="mt-12 grid gap-6 md:grid-cols-3">{testimonials.map((tm, i) => (<Reveal key={tm.name} delay={i * 0.1}><div className="h-full rounded-2xl bg-white p-6 shadow-sm"><Quote className="h-6 w-6 text-secondary-500" /><p className="mt-3 text-sm text-gray-900/80">{tm.text}</p><p className="mt-4 font-display text-sm font-semibold text-primary-900">{tm.name}</p></div></Reveal>))}</div></Section>);
}
