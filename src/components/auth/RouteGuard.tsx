...
export function RouteGuard({ children, requireAuth = true }: { children: React.ReactNode; requireAuth?: boolean }) {
  ...
  useEffect(() => { if (loading) return; if (requireAuth && !user) router.replace("/login?redirect=" + encodeURIComponent(pathname ?? "/")); if (!requireAuth && user) router.replace("/search"); }, [user, loading, requireAuth, router, pathname]);
