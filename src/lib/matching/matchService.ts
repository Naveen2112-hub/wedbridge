...
import { sanitizeText, getTodayKey } from "@/lib/utils";

export interface MatchScore { profile: ProfileDocument; score: number; reasons: string[]; }

const matchCache = new Map<string, { data: MatchScore[]; date: string }>();
...
export async function getAIMatches(userId: string, userProfile: ProfileDocument, max = 20): Promise<MatchScore[]> {
  ...
  try {
    ...
    const scored = profiles.map((p) => {
      ...
      if (p.district === userProfile.district) { score += 5; reasons.push("Same district"); }
      return { profile: p, score, reasons };
    });
    const result = scored.filter((m) => m.score > 0).sort((a, b) => b.score - a.score).slice(0, max);
    matchCache.set(userId, { data: result, date: todayKey });
    return result;
  } catch { return []; }
}
...
export async function sendInterest(fromUserId: string, fromUserName: string, toUserId: string, toProfileId: string, message?: string): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    const data: Omit<InterestDocument, "id" | "createdAt" | "updatedAt"> = { fromUserId, fromUserName, toUserId, toProfileId, status: "pending", message: message ? sanitizeText(message) : "" };
    ...
    return { ok: true };
  } catch (e) { return { ok: false, error: e instanceof Error ? e.message : "Failed to send interest" }; }
}
