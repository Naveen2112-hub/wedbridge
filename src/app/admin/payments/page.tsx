...
export default function AdminPaymentsPage() {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><h1 className="heading-md">Payments</h1><p className="text-lead mt-1 text-sm">Verify transactions and manage refunds.</p></div><div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-primary-100"><p className="text-xs text-muted">Total Revenue</p><p className="font-display text-xl font-bold text-green-700">{formatCurrency(totalRevenue)}</p></div></div>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select className="input max-w-[180px]" value={filter} onChange={(e) => setFilter(e.target.value as typeof filter)}><option value="all">All</option><option value="pending">Pending</option><option value="verified">Verified</option><option value="refunded">Refunded</option><option value="failed">Failed</option></select>
      </div>
