...
export function FaqSection() {
  ...
  return (<Section id="faq" className="bg-primary-50/30"><div className="mx-auto max-w-2xl text-center"><h2 className="heading-lg">{t("home.faq.title")}</h2><p className="text-lead mt-3">{t("home.faq.subtitle")}</p></div><div className="mx-auto mt-10 max-w-2xl space-y-3">{faqs.map((f, i) => (<div key={i} className="rounded-2xl bg-card shadow-soft"><button onClick={() => setOpen(open === i ? null : i)} className="flex w-full items-center justify-between gap-4 p-5 text-left"><span className="font-display text-base font-semibold text-primary-900">{f.q}</span><ChevronDown className={cn("h-5 w-5 flex-none text-muted transition", open === i && "rotate-180")} /></button>{open === i && <p className="px-5 pb-5 text-sm text-muted">{f.a}</p>}</div>))}</div></Section>);
}
