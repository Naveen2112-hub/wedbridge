...
import { createBackup, exportCollectionAsJSON, formatBackupSummary, type BackupResult, type BackupProgress } from "@/lib/admin/backupService";
...
export default function BackupPage() {
  ...
  const handleExportJSON = async (colName: string) => {
    ...
    if (typeof window !== "undefined") {
      ...
      link.href = url;
      link.download = `${colName}-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  return (
    <div>
      ...
      <div className="card mb-6 p-6">
        ...
        {progress && loading && (
          <div className="mt-4 flex items-center gap-2 text-sm text-gray-500">
            <RefreshCw className="h-3 w-3 animate-spin" />Scanning: {progress.collection} ({progress.current}/{progress.total})
          </div>
        )}
          ...
          <div className="mt-6">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {result.collections.map((c) => (
                <div key={c.name} className="flex items-center justify-between rounded-xl border border-gray-100 bg-gray-50 px-4 py-2">
                  <span className="text-sm font-medium text-gray-700">{c.name}</span>
                  <span className="text-sm text-gray-500">{c.count} docs</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="card p-6">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {collections.map((col) => (
            <button key={col} type="button" onClick={() => handleExportJSON(col)} disabled={exportingCol === col} className="flex items-center justify-between rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 disabled:opacity-50">
              <span>{col}</span>
              {exportingCol === col ? <RefreshCw className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />}
            </button>
          ))}
        </div>
      </div>
