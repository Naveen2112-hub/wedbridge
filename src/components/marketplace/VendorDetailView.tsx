...
export function VendorDetailView({ idPromise }: { idPromise: Promise<{ id: string }> }) {
  ...
  if (!vendor) return <div className="px-4 py-16 text-center text-gray-500">Vendor not found.</div>;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="card overflow-hidden">
        <div className="h-48 relative overflow-hidden bg-primary-100">{vendor.coverURL && <Image src={vendor.coverURL} alt={vendor.businessName} fill sizes="(max-width: 1024px) 100vw, 960px" className="object-cover" priority />}</div>
        <div className="p-6">
      ...
      <div className="mt-6 flex gap-2 overflow-x-auto">
        {tabs.map((t) => <button key={t.id} type="button" onClick={() => setTab(t.id)} className={`rounded-full px-4 py-2 text-sm font-medium transition ${tab === t.id ? "bg-primary-700 text-white" : "bg-white text-gray-500 ring-1 ring-primary-100 hover:text-primary-700"}`}>{t.label}</button>)}
      </div>
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          {tab === "gallery" && <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{gallery.length === 0 ? <p className="text-gray-500">No images.</p> : gallery.map((g) => <div key={g.id} className="relative aspect-square overflow-hidden rounded-xl bg-primary-100"><Image src={g.imageURL} alt={g.caption ?? "Gallery photo"} fill sizes="(max-width: 640px) 50vw, 33vw" className="object-cover" loading="lazy" /></div>)}</div>}
