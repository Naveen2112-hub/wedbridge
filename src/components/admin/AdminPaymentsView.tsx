  ...
  return (
    <div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <div className="card p-5"><CreditCard className="h-8 w-8 text-primary-600" /><p className="mt-2 font-display text-2xl font-bold text-primary-900">{payments.length}</p><p className="text-sm text-gray-500">Total Transactions</p></div>
        <div className="card p-5"><CreditCard className="h-8 w-8 text-green-600" /><p className="mt-2 font-display text-2xl font-bold text-primary-900">{successful.length}</p><p className="text-sm text-gray-500">Verified</p></div>
        <div className="card p-5"><CreditCard className="h-8 w-8 text-secondary-600" /><p className="mt-2 font-display text-2xl font-bold text-primary-900">{formatCurrency(total)}</p><p className="text-sm text-gray-500">Total Amount</p></div>
      </div>
