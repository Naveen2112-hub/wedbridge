...
export default function LoginPage() {
  ...
  const onSubmit = async (values: LoginValues) => {
    ...
    try {
      ...
      const { role } = await refreshRole(user.uid);
      routeAfterAuth(role);
    } catch (err) {
      ...
      <GlassCard>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex justify-end">
            <Link href="/forgot-password" className="text-xs font-medium text-secondary-700 hover:text-secondary-900">{t("auth.login.forgot")}</Link>
          </div>
...
async function refreshRole(uid: string) {
  ...
  const { collections } = await import("@/firebase/schema");
  if (!db) return { role: "user" as const };
  const snap = await getDoc(doc(db, collections.users, uid));
  return { role: (snap.data()?.role as string) ?? "user" };
}
