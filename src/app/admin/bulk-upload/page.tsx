...
import { CloudUpload as UploadCloud, FileSpreadsheet, CircleCheck as CheckCircle2, TriangleAlert as AlertTriangle, X, Loader as Loader2, ArrowLeft } from "lucide-react";
    ...
    for (const r of rows) {
      try {
        const profile: Omit<AdminProfile, "id" | "createdAt"> = {
          userId: String(r.userId ?? "bulk"), name: String(r.name ?? ""), gender: (String(r.gender) === "male" ? "male" : "female"), religion: String(r.religion || ""), caste: String(r.caste || ""), education: String(r.education || ""), occupation: String(r.occupation || ""), phone: String(r.phone || ""), district: String(r.district || ""), dob: String(r.dob || ""), status: "pending", premium: false, verified: false, featured: false, createdBy: "bulk",
        };
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r._row} className={cn("border-b border-primary-100 last:border-0", r._duplicate && "bg-amber-50/50")}>
                    ...
                    {headers.map((h) => <td key={h} className="p-3 text-xs">{r[h] ?? "—"}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="mt-2 text-xs text-muted"><AlertTriangle className="inline h-3 w-3 text-amber-500" /> Duplicate rows highlighted by phone number. Rollback clears the preview before save.</p>
        </div>
      )}
    </div>
  );
