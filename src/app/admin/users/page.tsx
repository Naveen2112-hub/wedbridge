...
export default function AdminUsersPage() {
  ...
  const act = async (u: AppUser, data: Partial<AppUser>) => { await updateUser(u.uid, data, admin); load(); };
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select className="input max-w-[180px]" value={filter} onChange={(e) => setFilter(e.target.value as typeof filter)}><option value="all">All Users</option><option value="active">Active</option><option value="blocked">Blocked</option><option value="verified">Verified</option><option value="unverified">Unverified</option></select>
      </div>
            ...
            filtered.map((u) => (
              <tr key={u.uid} className="border-b border-primary-100 last:border-0">
                ...
                <td className="p-3"><div className="flex flex-wrap gap-1">
                  ...
                  <button type="button" title="Reset Password" onClick={() => act(u, {} as never)} className="rounded-lg p-1.5 text-blue-600 hover:bg-blue-50"><KeyRound className="h-4 w-4" /></button>
      ...
      {view && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setView(null)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between"><h3 className="font-display text-lg font-bold text-primary-900">User Details</h3><button type="button" onClick={() => setView(null)} aria-label="Close"><X className="h-5 w-5 text-muted" /></button></div>
