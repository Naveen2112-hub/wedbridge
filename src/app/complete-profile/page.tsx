...
import { Heart, CircleAlert as AlertCircle, User, Phone, MapPin, Calendar, Shield, Users } from "lucide-react";
          ...
          <GlassCard>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label={t("auth.complete.name")} icon={User} error={errors.name && t("auth.error.required")}>
                  <input className="auth-input pl-10" {...register("name")} />
                </Field>
                <Field label={t("auth.complete.dob")} icon={Calendar} error={errors.dateOfBirth && t("auth.error.required")}>
                  <input type="date" className="auth-input pl-10" {...register("dateOfBirth")} />
                </Field>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label={t("auth.complete.religion")} icon={Shield} error={errors.religion && t("auth.error.required")}>
                  <input className="auth-input pl-10" {...register("religion")} />
                </Field>
                <Field label={t("auth.complete.caste")} icon={Users} error={errors.caste && t("auth.error.required")}>
                  <input className="auth-input pl-10" {...register("caste")} />
                </Field>
              </div>

              <div className="grid gap-4 sm:grid-cols-2">
                <Field label={t("auth.complete.district")} icon={MapPin} error={errors.district && t("auth.error.required")}>
                  <input className="auth-input pl-10" {...register("district")} />
                </Field>
                <Field label={t("auth.complete.phone")} icon={Phone} error={errors.phone && t("auth.error.required")}>
                  <input type="tel" className="auth-input pl-10" {...register("phone")} />
                </Field>
              </div>

              <Field label={t("auth.complete.email")} icon={User} error={errors.email && t("auth.error.email")}>
                <input type="email" className="auth-input pl-10" placeholder={user?.email ?? ""} {...register("email")} />
              </Field>

              <div>
                <label className="label">{t("auth.complete.photo")}</label>
                <input type="url" className="auth-input" placeholder="https://…" {...register("photoURL")} />
              </div>
                ...
                <div className="space-y-2">
                  {visibilityOptions.map((o) => (
                    <label key={o.value} className={cn("flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 text-sm transition", visibility === o.value ? "border-secondary bg-secondary-50 text-primary-900" : "border-primary-100 bg-white text-muted hover:border-primary-300")}>
                      ...
                      <span className={cn("flex h-4 w-4 flex-none items-center justify-center rounded-full border", visibility === o.value ? "border-secondary bg-secondary" : "border-primary-200")}>
                        {visibility === o.value && <span className="h-2 w-2 rounded-full bg-white" />}
                      </span>
                      {t(o.labelKey)}
                    </label>
                  ))}
