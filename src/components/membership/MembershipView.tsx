...
import { PLANS } from "@/lib/membership/membershipService";
...
export function MembershipView() {
  ...
  const handleSubscribe = (plan: typeof PLANS[number]) => {
    toast(`Redirecting to payment for ${plan.name} plan…`, "info");
  };
        ...
        {PLANS.map((plan, i) => (
          <div key={plan.id} className={`card relative p-8 ${i === 1 ? "ring-2 ring-secondary-400" : ""}`}>
            ...
            <ul className="mt-6 space-y-3">
              {plan.features.map((f) => (
                <li key={f} className="flex items-start gap-2 text-sm"><Check className="mt-0.5 h-4 w-4 flex-none text-green-600" />{f}</li>
              ))}
            </ul>
