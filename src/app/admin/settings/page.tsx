...
import { defaultSettings, type SiteSettings } from "@/lib/admin/schema";
      ...
      <div className="space-y-6">
        ...
        <Section title="Homepage Banners">
          <div className="sm:col-span-2 space-y-2">
            {settings.homepageBanners.map((b, i) => (
              <div key={i} className="flex gap-2"><input className="input" value={b} onChange={(e) => update({ homepageBanners: settings.homepageBanners.map((x, idx) => idx === i ? e.target.value : x) })} /><button type="button" onClick={() => update({ homepageBanners: settings.homepageBanners.filter((_, idx) => idx !== i) })} className="btn-outline px-3"><Trash2 className="h-4 w-4" /></button></div>
            ))}
