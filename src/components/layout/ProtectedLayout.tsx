...
export function ProtectedLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-grain">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-primary-100 bg-white/80 backdrop-blur-md lg:flex lg:flex-col">
        <SidebarContent pathname={pathname} t={t} onLogout={handleLogout} userName={user?.displayName ?? user?.email ?? ""} />
      </aside>
      {open && (
        <aside className="fixed inset-y-0 left-0 z-50 w-64 border-r border-primary-100 bg-white shadow-card lg:hidden">
      ...
      <div className="flex flex-1 flex-col lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-primary-100 bg-background/90 px-4 backdrop-blur-md lg:px-8">
          ...
          <Link href="/" className="flex items-center gap-2.5" aria-label="WedBridge home">
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-white shadow-soft"><Heart className="h-4 w-4" fill="currentColor" /></span>
            <span className="font-display text-lg font-semibold text-primary-900">Wed<span className="text-secondary-600">Bridge</span></span>
          </Link>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between gap-2 px-5 py-5">
        ...
        {onClose && <button type="button" onClick={onClose} className="rounded-full p-1.5 text-primary-900" aria-label="Close menu"><X className="h-5 w-5" /></button>}
      </div>
      ...
      <nav className="flex-1 space-y-1 px-3 py-2">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onClose} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition", active ? "bg-primary-50 text-primary-900" : "text-ink/70 hover:bg-primary-50/60 hover:text-primary-900")}>
      ...
      <div className="border-t border-primary-100 p-3">
        <button type="button" onClick={onLogout} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-ink/70 transition hover:bg-accent-50 hover:text-accent-700"><LogOut className="h-4 w-4" />{t("nav.logout" as never)}</button>
      </div>
    </div>
  );
}
