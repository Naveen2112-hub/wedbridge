...
export default function CompleteProfilePage() {
  ...
  const { register, handleSubmit, watch, formState: { errors, isSubmitting } } = useForm<CompleteProfileValues>({ resolver: zodResolver(completeProfileSchema), defaultValues: { name: user?.displayName ?? "", gender: "male", dateOfBirth: "", religion: "", caste: "", district: "", phone: "", email: "", photoURL: "", contactVisibility: "after_accept" } });
  if (loading) return null;
  if (!user) { router.replace("/login"); return null; }
  const visibility = watch("contactVisibility");
  const onSubmit = async (values: CompleteProfileValues) => { setAuthError(null); try { await completeProfile(values); router.push("/dashboard"); } catch (err) { setAuthError(mapAuthError(err)); } };
  const visibilityOptions: { value: ContactVisibility; labelKey: "auth.complete.visibility.everyone" | "auth.complete.visibility.afterAccept" | "auth.complete.visibility.premiumOnly" }[] = [
    { value: "everyone", labelKey: "auth.complete.visibility.everyone" }, { value: "after_accept", labelKey: "auth.complete.visibility.afterAccept" }, { value: "premium_only", labelKey: "auth.complete.visibility.premiumOnly" },
  ];
  return (
    <PublicLayout>
      <section className="container-page section-pad"><div className="mx-auto max-w-2xl"><GlassCard>
        <div className="mb-8 text-center"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-primary-50 text-primary-800"><Heart className="h-6 w-6" fill="currentColor" /></span><h1 className="heading-md mt-4">{t("auth.complete.title")}</h1><p className="text-lead mt-2">{t("auth.complete.subtitle")}</p></div>
