...
import { ChartBar as FileBarChart, IndianRupee, Heart } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        {(["daily", "weekly", "monthly", "yearly"] as const).map((p) => <button key={p} type="button" onClick={() => setPeriod(p)} className={cn("rounded-full px-4 py-1.5 text-sm font-medium capitalize transition", period === p ? "bg-primary-600 text-white" : "bg-white text-muted ring-1 ring-primary-100 hover:text-primary-700")}>{p}</button>)}
      </div>
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-primary-100"><div className="flex items-center gap-2 text-muted"><IndianRupee className="h-4 w-4" /><span className="text-sm">Total Revenue</span></div><p className="mt-2 font-display text-2xl font-bold text-primary-900">{formatCurrency(totalRevenue)}</p></div>
      ...
      <div className="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-primary-100">
        ...
        <div className="mt-4 space-y-3">
          {rows.map((r) => <div key={r.date} className="flex items-center gap-3"><span className="w-32 flex-none text-xs text-muted">{r.date}</span><div className="h-6 flex-1 overflow-hidden rounded-lg bg-primary-50"><div className="h-full rounded-lg bg-primary-500 transition-all" style={{ width: `${(r.revenue / maxRev) * 100}%` }} /></div><span className="w-24 flex-none text-right text-sm font-semibold text-primary-900">{formatCurrency(r.revenue)}</span></div>)}
        </div>}
      </div>
    </div>
  );
