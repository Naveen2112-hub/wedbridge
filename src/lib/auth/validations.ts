...
export const completeProfileSchema = z.object({ name: z.string().min(2, "required"), gender: z.enum(["male", "female", "other"]), dateOfBirth: z.string().min(1, "required"), religion: z.string().min(1, "required"), caste: z.string().min(1, "required"), district: z.string().min(1, "required"), phone: z.string().min(6, "required"), email: z.string().email("email").optional().or(z.literal("")), photoURL: z.string().optional(), contactVisibility: z.enum(["everyone", "after_accept", "premium_only"]) });
