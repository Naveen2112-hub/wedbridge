          ...
          <Reveal key={s.couple} delay={i * 0.08}>
            <article className="group h-full overflow-hidden rounded-2xl bg-card shadow-card transition hover:-translate-y-1 hover:shadow-glow">
              <div className="relative aspect-[5/4] overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3 right-3 text-white"><p className="flex items-center gap-1.5 font-display text-lg font-semibold"><Heart className="h-4 w-4 text-accent" fill="currentColor" />{s.couple}</p><p className="text-xs text-white/80">{s.city}</p></div>
              </div>
