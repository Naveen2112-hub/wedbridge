    ...
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      <Reveal>
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-lg text-neutral-600">
            Join us in building Tamil Nadu&apos;s most trusted matrimony platform. We are looking for
      ...
      <Reveal delay={100}>
        <div className="mt-12 grid gap-4 sm:grid-cols-3">
          {[
            { icon: Briefcase, label: "Open Roles", value: "5" },
            { icon: Code, label: "Tech Stack", value: "Next.js + Firebase" },
            { icon: BarChart, label: "Growth", value: "50K+ Users" },
          ].map((s) => (
            <div key={s.label} className="rounded-2xl border border-neutral-200 bg-white p-6 text-center">
              <s.icon className="mx-auto h-8 w-8 text-rose-600" />
              ...
              <div className="rounded-2xl border border-neutral-200 bg-white p-6 transition hover:shadow-md">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-neutral-500">{job.type} · {job.location}</p>
                  </div>
                  <a
                    href={`mailto:support@wedbridge.in?subject=Application: ${encodeURIComponent(job.title)}`}
      ...
      <Reveal delay={200}>
        <div className="mt-12 rounded-2xl bg-rose-600 p-8 text-center">
          ...
          <div className="mt-4 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <a href="mailto:support@wedbridge.in" className="flex items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-sm font-semibold text-rose-600">
              <Mail className="h-4 w-4" /> support@wedbridge.in
            </a>
            <a href="tel:+916383109341" className="flex items-center gap-2 rounded-xl border border-white/30 px-5 py-2.5 text-sm font-semibold text-white">
