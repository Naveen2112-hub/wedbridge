      ...
      <div className="container-page py-16">
        <div className="grid gap-12 lg:grid-cols-12">
          <div className="lg:col-span-4">
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex gap-2">
                {socials.map((s) => (
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="flex h-10 w-10 items-center justify-center rounded-full border border-primary-100 bg-white text-primary-800 transition hover:border-secondary hover:text-secondary-700"><s.icon className="h-4 w-4" /></a>
                ))}
              </div>
            </div>
          </div>
          <div className="lg:col-span-2">
            ...
            <ul className="mt-4 space-y-2.5">{quickLinks.map((l) => (<li key={l.href}><Link href={l.href} className="link-quiet text-sm">{t(l.key)}</Link></li>))}</ul>
          </div>
          <div className="lg:col-span-3">
            <p className="font-display text-base font-semibold text-primary-900">{t("footer.support")}</p>
            <ul className="mt-4 space-y-2.5">{supportLinks.map((l) => (<li key={l.href}><Link href={l.href} className="link-quiet text-sm">{t(l.key)}</Link></li>))}</ul>
          </div>
          <div className="lg:col-span-3">
            <p className="font-display text-base font-semibold text-primary-900">{t("footer.legal")}</p>
            <ul className="mt-4 space-y-2.5">{legalLinks.map((l) => (<li key={l.href}><Link href={l.href} className="link-quiet text-sm">{t(l.key)}</Link></li>))}</ul>
          </div>
        </div>
