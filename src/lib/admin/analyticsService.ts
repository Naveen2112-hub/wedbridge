...
export async function getAnalytics(): Promise<AdminAnalytics> {
  ...
  try {
    const [usersSnap, profilesSnap, vendorsSnap, paymentsSnap, interestsSnap] = await Promise.all([
      getDocs(query(collection(db, collections.users), limit(1000))).catch(() => ({ empty: true, docs: [] })),
      getDocs(query(collection(db, collections.profiles), limit(1000))).catch(() => ({ empty: true, docs: [] })),
      getDocs(query(collection(db, collections.vendors), limit(1000))).catch(() => ({ empty: true, docs: [] })),
      getDocs(query(collection(db, collections.payments), limit(1000))).catch(() => ({ empty: true, docs: [] })),
      getDocs(query(collection(db, collections.interests), limit(1000))).catch(() => ({ empty: true, docs: [] })),
    ]);
    ...
    const isToday = (ts: unknown) => { if (!ts || typeof ts !== "object") return false; const d = (ts as { toMillis?: () => number }).toMillis?.(); if (!d) return false; return d >= todayStart.getTime(); };
    const isThisMonth = (ts: unknown) => { if (!ts || typeof ts !== "object") return false; const d = (ts as { toMillis?: () => number }).toMillis?.(); if (!d) return false; return d >= monthStart.getTime(); };

    const todayRevenue = payments.filter((p) => p.status === "verified" && isToday(p.createdAt)).reduce((s, p) => s + Number(p.amount ?? 0), 0);
    const monthlyRevenue = payments.filter((p) => p.status === "verified" && isThisMonth(p.createdAt)).reduce((s, p) => s + Number(p.amount ?? 0), 0);
...
export async function getReport(period: "daily" | "weekly" | "monthly" | "yearly"): Promise<ReportRow[]> {
  ...
  try {
    ...
    const fmt = period === "yearly" ? { year: "numeric" } as const : period === "monthly" ? { month: "short", year: "numeric" } as const : period === "weekly" ? { month: "short", day: "numeric" } as const : { month: "short", day: "numeric" } as const;
    for (const p of payments) {
      const ts = (p.createdAt as { toMillis?: () => number } | undefined)?.toMillis?.();
      if (!ts) continue;
      const d = new Date(ts);
      const key = d.toLocaleDateString("en-US", fmt);
