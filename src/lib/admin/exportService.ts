...
export async function exportUsers(format: ExportFormat, max = 1000): Promise<void> {
  ...
  try {
    ...
    else downloadHTMLAsPDF(`<!DOCTYPE html><html><head><title>Users Export</title><style>body{font-family:Arial}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;font-size:12px}th{background:#f5f5f5}</style></head><body><h1>WedBridge Users - ${ts}</h1><p>Total: ${users.length}</p><table><thead><tr>${headers.map((h) => `<th>${h}</th>`).join("")}</tr></thead><tbody>${rows.map((r) => `<tr>${r.map((c) => `<td>${c ?? ""}</td>`).join("")}</tr>`).join("")}</tbody></table></body></html>`);
...
export async function exportVendors(format: ExportFormat, max = 1000): Promise<void> {
  ...
  try {
    ...
    else downloadHTMLAsPDF(`<!DOCTYPE html><html><head><title>Vendors Export</title><style>body{font-family:Arial}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;font-size:12px}th{background:#f5f5f5}</style></head><body><h1>WedBridge Vendors - ${ts}</h1><p>Total: ${vendors.length}</p><table><thead><tr>${headers.map((h) => `<th>${h}</th>`).join("")}</tr></thead><tbody>${rows.map((r) => `<tr>${r.map((c) => `<td>${c ?? ""}</td>`).join("")}</tr>`).join("")}</tbody></table></body></html>`);
