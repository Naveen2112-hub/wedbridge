...
export function AdminGuard({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (loading) return; if (!user) { router.replace("/admin/login"); return; } if (role !== "admin") { setDenied(true); (async () => { await logout(); setTimeout(() => router.replace("/admin/login"), 1500); })(); } }, [user, role, loading, router, logout]);
