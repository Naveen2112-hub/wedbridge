...
import {
  Search, User, Store, Sparkles, Bell, Crown, Heart, ArrowRight,
  MessageCircle, Eye, Users, CheckCircle, TrendingUp, Settings,
  Loader as Loader2, Star,
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    (async () => {
      const [s, n] = await Promise.all([
        ...
        getUserNotifications(user.uid, 5),
      ]);
      setSub(s);
      setNotifications(n);
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-neutral-200 bg-white p-5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-neutral-500">Membership</span>
          </div>
          <p className="mt-2 text-lg font-bold capitalize text-neutral-900">{tier}</p>
          {daysLeft > 0 && <p className="text-xs text-neutral-400">{daysLeft} days remaining</p>}
        </div>
        <div className="rounded-2xl border border-neutral-200 bg-white p-5">
          <div className="flex items-center gap-2">
            <CheckCircle className="h-5 w-5 text-green-500" />
            <span className="text-sm font-medium text-neutral-500">Profile</span>
          </div>
          <p className="mt-2 text-lg font-bold text-neutral-900">{profileComplete ? "Complete" : "Incomplete"}</p>
          {!profileComplete && <Link href="/complete-profile" className="text-xs text-rose-600 hover:underline">Complete now</Link>}
        </div>
        <div className="rounded-2xl border border-neutral-200 bg-white p-5">
          <div className="flex items-center gap-2">
            <Bell className="h-5 w-5 text-blue-500" />
            <span className="text-sm font-medium text-neutral-500">Notifications</span>
          </div>
          <p className="mt-2 text-lg font-bold text-neutral-900">{unreadCount} unread</p>
          <Link href="/notifications" className="text-xs text-rose-600 hover:underline">View all</Link>
        </div>
        <div className="rounded-2xl border border-neutral-200 bg-white p-5">
          <div className="flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-violet-500" />
            <span className="text-sm font-medium text-neutral-500">AI Matches</span>
          </div>
          <p className="mt-2 text-lg font-bold text-neutral-900">Smart</p>
          <Link href="/ai-matches" className="text-xs text-rose-600 hover:underline">View matches</Link>
        </div>
      </div>
        ...
        <div className="mt-6 rounded-2xl border border-neutral-200 bg-white p-5">
          ...
          <div className="mt-3 space-y-2">
            {notifications.slice(0, 3).map((n) => (
              <div key={n.id} className="flex items-start gap-3 border-b border-neutral-50 pb-2">
                <Bell className="mt-0.5 h-4 w-4 flex-none text-neutral-400" />
                <div>
