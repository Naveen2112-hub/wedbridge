...
import { searchProfiles, type SearchFilters } from "@/lib/profile/profileService";
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex gap-3">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><input className="input pl-10" placeholder="Search by name…" onChange={(e) => setFilters((f) => ({ ...f, caste: e.target.value }))} /></div>
          ...
          {results.map((p) => (
            <div key={p.id} className="card overflow-hidden transition hover:shadow-md">
              <div className="flex gap-4 p-4">
                <div className="h-20 w-20 flex-none overflow-hidden rounded-xl bg-primary-100">{p.photoURL && <img src={p.photoURL} alt={p.name} className="h-full w-full object-cover" loading="lazy" />}</div>
                <div className="flex-1">
                  <div className="flex items-center gap-1.5"><h3 className="font-display text-lg font-semibold text-primary-900">{p.name}</h3>{p.verified && <BadgeCheck className="h-4 w-4 text-green-600" />}{p.premium && <Crown className="h-4 w-4 text-secondary-500" />}</div>
                  <p className="text-xs text-muted">{p.religion} · {p.caste ?? "Any"}</p>
                  <p className="mt-1 flex items-center gap-1 text-xs text-muted"><MapPin className="h-3 w-3" />{p.district ?? p.city ?? "Tamil Nadu"}</p>
                  <p className="mt-1 text-xs text-muted">{p.occupation ?? ""}</p>
                </div>
              </div>
              <div className="border-t border-primary-50 p-3"><button type="button" onClick={() => handleInterest(p)} disabled={sending === p.id} className="btn-primary w-full text-sm">{sending === p.id ? <Loader2 className="h-4 w-4 animate-spin" /> : "Send Interest"}</button></div>
            </div>
          ))}
        </div>
      )}
