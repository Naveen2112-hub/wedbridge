  ...
  return (
    ...
    >
      <nav className="container-page flex h-16 items-center justify-between lg:h-20">
              ...
              <li key={item.href}>
                <Link
                  ...
                  className={cn(
                    ...
                    active ? "bg-primary-50 text-primary-900" : "text-ink/70 hover:text-primary-900",
                  )}
                >
                  {t(item.key)}
                </Link>
        ...
        <button
          type="button"
          className="inline-flex items-center justify-center rounded-full p-2 text-primary-900 lg:hidden"
          onClick={() => setOpen((v) => !v)}
          ...
          <div className="container-page flex flex-col gap-1 py-4">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-xl px-3 py-2.5 text-sm font-medium text-ink/80 hover:bg-primary-50 hover:text-primary-900"
              >
                {t(item.key)}
              </Link>
            ))}
