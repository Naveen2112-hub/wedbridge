...
export async function checkDuplicate(
  ...
  try {
    const db = getDb();
    const snap = await db.collection(collections.ocrImports).where("status", "in", ["approved", "pending_review"]).get();
...
export async function storeImport(
  ...
  const db = getDb();
  const docRef = await db.collection(collections.ocrImports).add({
    ...record,
    importTime: new Date(),
  });
  return { id: docRef.id };
...
export async function getImports(
  ...
  try {
    ...
    let snap;
    if (statusFilter) {
      snap = await db.collection(collections.ocrImports).where("status", "==", statusFilter).orderBy("importTime", "desc").limit(100).get();
    } else {
      snap = await db.collection(collections.ocrImports).orderBy("importTime", "desc").limit(100).get();
    }
...
export async function updateImportStatus(
  ...
  await db.collection(collections.ocrImports).doc(importId).update(update);
}
...
export async function getImportById(importId: string): Promise<OcrImportRecord | null> {
  const db = getDb();
  const snap = await db.collection(collections.ocrImports).doc(importId).get();
...
export async function findUserByChatId(
  ...
  try {
    const db = getDb();
    const snap = await db.collection(collections.users).where("telegramChatId", "==", chatId).limit(1).get();
