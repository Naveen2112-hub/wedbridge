  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {profiles.map((p) => (
          <Link key={p.uid} href={`/profile/${p.uid}`} className="group overflow-hidden rounded-xl bg-white shadow-md transition hover:shadow-lg">
            ...
            <div className="flex items-center justify-between p-3"><div><p className="text-sm font-semibold text-primary-900">{p.name}</p><p className="text-xs text-gray-500">{p.district}</p></div><ChevronRight className="h-4 w-4 text-gray-500 transition group-hover:translate-x-0.5" /></div>
          </Link>
        ))}
      </div>
    </div>
