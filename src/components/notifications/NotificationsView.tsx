    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6"><h2 className="heading-sm">Interests Received</h2><div className="mt-3 space-y-3">
          {interests.map((i) => (
            <div key={i.id} className="card flex items-center justify-between p-4">
              <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100 text-primary-700"><Heart className="h-5 w-5" /></span><div><p className="font-medium text-primary-900">{i.fromUserName}</p><p className="text-xs text-muted">{formatDate(i.createdAt as unknown as string)}</p>{i.message && <p className="mt-1 text-sm text-muted">&ldquo;{i.message}&rdquo;</p>}</div></div>
              {i.status === "pending" ? (
      ...
      {notifs.length > 0 && (
        <div className="mt-6"><h2 className="heading-sm">Announcements</h2><div className="mt-3 space-y-3">
          {notifs.map((n) => <div key={n.id} className="card p-4"><p className="font-medium text-primary-900">{n.title}</p><p className="mt-1 text-sm text-muted">{n.message}</p><p className="mt-1 text-xs text-muted">{formatDate(n.createdAt as unknown as string)}</p></div>)}
        </div></div>
      )}
